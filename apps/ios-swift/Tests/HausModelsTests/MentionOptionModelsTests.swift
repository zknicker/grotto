...
final class MentionOptionModelsTests: XCTestCase {
    func testDecodesChannelMentionOption() throws {
        ...
        let option = try HausJSON.decoder().decode(MentionOption.self, from: Data(json.utf8))
        ...
        let human = try HausJSON.decoder().decode(
            MentionOption.self,
            from: Data(json
    ...
    func testDecodesSkillMentionOptionAndToleratesAnUnknownKind() throws {
        ...
        let roster = try HausJSON.decoder().decode(MentionOptions.self, from: Data(json.utf8))
