...
const privilegedActivationHelper = '/usr/local/libexec/haus/activate-haus-server';
const serverLabel = 'system/com.haus.server';
const serverPlist = '/Library/LaunchDaemons/com.haus.server.plist';
...
interface ActivateHausReleaseInput {
    deployRoot: string;
...
export async function activateHausRelease(input: ActivateHausReleaseInput) {
...
export async function assertPrivilegedActivationHelper(
    ...
    await assertRootOwnedComponents(
        [
            ...
            ['/usr/local/libexec/haus', directoryType],
...
async function main() {
    ...
    if (!(sourceRevision && extra.length === 0)) {
        throw new Error('Usage: activate-haus-server FULL_GIT_SHA');
    }

    const activatedRevision = await activateHausRelease({
