...
beforeAll(async () => {
    ...
    owner = createHausClient(harness, await harness.clerk.mintSessionToken('download_owner'));
    outsider = createHausClient(harness, await harness.clerk.mintSessionToken('download_outsider'));
...
async function download(client: HausClient, scopedServerId: string, scopedAttachmentId: string) {
    return await fetch(
        new URL(`/attachments/${scopedServerId}/${scopedAttachmentId}`, harness.url),
        headers(client)
    );
...
function headers(client: HausClient) {
    return {
        headers: {
