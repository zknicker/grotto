...
import { catchUpReminderChanges, laterReminderCursor } from './reminder-event-cursor.ts';

export type ReminderConnectionState = 'connected' | 'connecting' | 'reconnecting';

export function useReminderEvents(serverId: string) {
    const utils = grottoTrpc.useUtils();
    const stateRef = React.useRef({ cursor: '0', serverId });
    const [connectionState, setConnectionState] =
        React.useState<ReminderConnectionState>('connecting');
