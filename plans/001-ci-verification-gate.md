...
- `.github/workflows/deploy.yml` — single `deploy` job on a **self-hosted
  ...
  separate deployed checkout at `/Users/zknicker/srv/haus` and runs
  ...
  jobs:
    deploy:
      ...
      steps:
        ...
        - name: Build and deploy
          working-directory: /Users/zknicker/srv/haus
          run: |
            git pull --ff-only
...
- Per-workspace test lanes (from each `package.json`):
  ...
  - `packages/haus-api`, `packages/haus-sdk`: `"test": "bun test"`.
...
| Server tests  | `bun run --filter @haus/server test`  | all pass        |
...
- `scripts/deploy-local.sh` and anything under `/Users/zknicker/srv/haus` —
...
**Verify**: `bun run --filter @haus/website test` → all pass (or lane
...
```json
"test": "bun run --filter @haus/api test && bun run --filter @haus/sdk test && bun run --filter @haus/claude-usage test && bun run --filter @haus/codex-usage test && bun run --filter @haus/server test && bun run --filter @haus/website test"
```
...
it `working-directory: /Users/zknicker/srv/haus`. The runner is the same
