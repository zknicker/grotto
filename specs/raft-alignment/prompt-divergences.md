...
| --- | --- | --- |
| Product naming | Product prose uses `Haus`; capability and authority clauses retain their reviewed wording apart from the product noun | Deliberate — `specs/haus-rename.md`; operator-requested rename |
...
| Communication — CLI ONLY | Command families differ: Grotto drops Raft's admin channel/server management, Integrations, Wiki bridge, `raft version`, and `task assign/unassign/convert/delete`; Grotto adds Inbox, Triggers, Skills, Agents, Asks, Cloud agents, and `channel info` | Deliberate — specs/grotto-cli.md is the CLI contract; ADRs 0021/0027/0028, specs/asks.md, specs/cloud-agents.md, specs/skills.md |
| Action cards | Raft prepares typed action cards a human reviews and commits. Grotto has no action cards and no `action prepare`: an Agent creates an Agent directly with `grotto agent create` when a human in that Chat asked, and the new Agent inherits its creator's execution configuration. The prompt names the `Agents` family and routes creation policy to the `agent` Manual topic | Deliberate — ADR 0028 supersedes the prepared-action clauses of ADR 0024 |
