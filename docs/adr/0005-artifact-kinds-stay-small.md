...
Haus chat artifacts use a small product enum: `document`, `image`, and
`custom`.

We are not keeping separate artifact kinds for code, diffs, files, charts, or
plain text because those are not distinct Haus product surfaces today:
charts belong to Widgets, diffs are not a Haus review surface, code
...
such as Wiki pages or workspace files; those targets do not expand
`ChatArtifact.kind`.
