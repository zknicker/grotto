...
test('Cloud Agent work stores bounded, provider-safe execution state', () => {
    ...
    expect(cloudAgentWorkSchema.safeParse({ ...work, repository: 'grotto' }).success).toBe(false);
...
test('Computer inventory reports Cloud Agent provider readiness on protocol 14', () => {
    expect(
