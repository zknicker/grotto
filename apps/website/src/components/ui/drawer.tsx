...
export function DrawerHeader({
    ...
    const defaultProps = {
        className: cn(
            'flex flex-col gap-1 px-6 py-5 pe-28 in-[[data-slot=drawer-popup]:has([data-slot=drawer-panel])]:pb-3 max-sm:pb-4',
...
export function DrawerFooter({
    ...
    const defaultProps = {
        className: cn(
            ...
            !allowSelection && 'cursor-default',
            variant === 'default' &&
                'border-t bg-legacy-muted/72 pt-4 pb-[calc(env(safe-area-inset-bottom,0px)+--spacing(4))]',
            variant === 'bare' &&
...
export function DrawerMenuTrigger({
    ...
    return (
        ...
        >
            {children}
            <Icon className="ms-auto -me-0.5 opacity-80" icon={ArrowRight01Icon} size={16} />
        </DrawerTrigger>
    );
}
