...
/// store is non-persistent, and the CSP below pins the only reachable external
/// source to the Chart.js CDN entry.
public enum VisualSandboxDocument {
    /// The one allowed external script, pinned by version. Bumping the pin is a
    /// deliberate supply-chain decision shared with the web card and the seeded
    /// skill guidance (docs/internals/widgets.md).
    ...
    public static let sizeMessageName = "grottoVisualSize"
    ...
    * { box-sizing: border-box; }
    ...
    var report = function () {
            ...
            parent.postMessage({ height: height, type: 'grotto-visual-size' }, '*');
            var webkit = window.webkit;
            if (webkit && webkit.messageHandlers && webkit.messageHandlers.grottoVisualSize) {
                webkit.messageHandlers.grottoVisualSize.postMessage(height);
            }
        };
