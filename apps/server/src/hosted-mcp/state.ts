...
export async function setHostedMcpGrant(
    ...
    if (!member || (access.role !== 'owner' && access.role !== 'admin')) {
        throw new HostedMcpDeniedError('Only a Server Owner or Admin can change Agent access.');
    }
    const [scope] = await db
        ...
        .limit(1);
    if (!scope) {
        throw new HostedMcpDeniedError('The Agent and MCP connection must share a Server.');
    }
    if (input.enabled && !scope.connected) {
        throw new HostedMcpDeniedError('Connect this MCP server before enabling Agent access.');
    }
    const key = {
