...
import { mergeHostedArtifactTarget } from './use-hosted-chat-artifact-panel.ts';
...
test('keeps same-path workspace artifacts separate for different authors', () => {
    ...
    const withFirst = mergeHostedArtifactTarget({ activeKey: null, targets: [] }, first);
    const withBoth = mergeHostedArtifactTarget(withFirst, second);
...
test('morphs only the selected author workspace tab to a new file', () => {
    ...
    const initial = mergeHostedArtifactTarget(
        mergeHostedArtifactTarget({ activeKey: null, targets: [] }, firstAuthor),
        secondAuthor
    );

    const result = mergeHostedArtifactTarget(initial, nextFirstAuthor);
