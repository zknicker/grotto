...
/// JSON coding configured for Haus's ISO-8601 timestamps.
///
...
/// Server.
public enum HausJSON {
    public static func decoder() -> JSONDecoder {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .custom { decoder in
            let container = try decoder.singleValueContainer()
            let value = try container.decode(String.self)
            guard let date = HausISO8601.date(from: value) else {
                throw DecodingError.dataCorruptedError(
                    in: container,
                    debugDescription: "Expected a Haus ISO-8601 timestamp."
                )
            }
            return date
        }
    ...
    public static func encoder() -> JSONEncoder {
        ...
        encoder.dateEncodingStrategy = .custom { date, encoder in
            ...
            try container.encode(HausISO8601.string(from: date))
        }
        return encoder
    }
}

public enum HausISO8601 {
