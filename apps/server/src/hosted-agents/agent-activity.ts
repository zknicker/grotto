import type { HostedAgentActivityEvent, HostedAgentActivityFrame } from '@tavern/api';
...
interface AppendActivityInput {
    agentId: string;
    category: HostedAgentActivityFrame['category'];
    occurredAt?: string;
    phase: HostedAgentActivityFrame['phase'];
...
): Promise<HostedAgentActivityEvent | null> {
...
export async function recordComputerAgentActivity(
    ...
    input: {
        computerId: string;
        frame: HostedAgentActivityFrame;
        serverId: string;
    }
): Promise<HostedAgentActivityEvent | null> {
...
): Promise<{ event: HostedAgentActivityEvent; inserted: boolean } | null> {
