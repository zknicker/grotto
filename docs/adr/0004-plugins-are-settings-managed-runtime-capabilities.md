...
Grotto Plugins are built-in, manifest-declared capability bundles configured
and repaired through Grotto settings. Agents may read Plugin health and use
...
Grotto-wide settings flow, not a Plugin-specific shortcut.
...
Core Grotto owns the Plugin host boundary: lifecycle, enablement, Service
...
instead of spreading operation names through generic Grotto docs.
...
Grotto does not expose individual Plugin tool toggles.
...
controls inside historical renders are disabled; Grotto does not use fallback
...
Google's OAuth client id is Grotto-owned infrastructure config, not a Plugin
setting. Runtime reads the desktop client id and installed-app client secret from
environment supplied by Grotto development or release packaging and stores only
