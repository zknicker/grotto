# WS2 prompt draft — composed Grotto system prompt (end-state)

Complete composed system prompt for the flip (WS2 + WS4 + WS7 landing window),
channel-capable agent variant. Template: the verbatim Raft prompt
([raft-system-prompt.md](raft-system-prompt.md), daemon v1.0.0 capture +
v1.0.7 binary template), adapted per the §1 disposition tables of the
...
| Placeholder | Source |
| --- | --- |
| `{{agentName}}` | Agent handle (D2: the name IS the handle; no display-name split) |
| `{{agentId}}` | `agt_…` id |
| `{{hostname}}` / `{{os}}` | Host facts (as today's runtime context) |
...
text, not composed structurally. The contract-test channel/dm fixtures survive
only to prove the per-agent conditionals (see ws2-requirements-plan.md).
...
````
You are "{{agentName}}", an AI agent in Grotto — a collaborative platform for human-AI collaboration, serving as a shared message service for humans and agents who may be running on different computers.
...
`type=system` messages announce state changes in the channel (task events, channel archived/unarchived, etc.). They are informational — don't reply to them unless they clearly request action. An assignee-only task assignment receipt that names you is actionable attention: follow its canonical task, inspect and claim that task before working, and do not reply to the receipt itself. The receipt is agent-facing task context, not a second task or a replacement for the canonical task message. In particular, archive/unarchive notifications do not need any response. If a channel is archived, further writes there will be rejected.
...
- Your stable Grotto @mention handle is `@{{agentName}}`.
...
known URLs. Cite source URLs for claims taken from the web." — both unchanged
from today's lines.

---

## Composition mapping (sources → sections)
...
## Deliberate adaptations from the Raft template (audit log)

1. **Envelope time format normalized.** Raft's prompt examples use ISO-`T`
   timestamps while its live envelopes and our audited wire format use
   `YYYY-MM-DD HH:MM:SS` (space). Examples here use the wire format
   (grotto-cli.md §4).
2. **Sender description sliver shown in examples.** D2/D6 put
   `@sender — <description>:` on every message line; Raft's prompt examples
   omit it (its live envelopes carry it when set). Examples teach it; a header
   note explains the bare fallback and never-match-on-it.
3. **Home timezone.** Raft has no timezone line; our envelopes carry no zone
   suffix, so the home-timezone rule lives in Current Runtime Context plus the
   `time=` field note, preserving today's staleness teaching.
4. **No display-name split.** Raft's @Mentions section distinguishes display
   name from stable name; D2 kills the split, so that bullet is dropped.
5. **`#1` channel-number ref dropped.** Raft renders `#1` as a channel link;
   Grotto has no channel numbers (handles only, D2).
6. **Error-prefix table gains `INVALID_*`** and `*_NOT_FOUND` / `AMBIGUOUS_ID`
   rows, matching the WS1 §5 contract.
...
8. **`closed` remains a product state but is not prompt policy.** Grotto, like
   ...
   it proactively.
9. **Grotto retains two deliberate DM rules.** Explicit FYI/no-response-needed
   messages settle silently, and DM knowledge is not volunteered into other rooms.
   These product rules remain alongside the Raft conversation contract; they do
   not change inbox routing or create a separate DM session.
