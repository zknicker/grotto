...
Haus Plugins are built-in, manifest-declared capability bundles configured
and repaired through Haus settings. Agents may read Plugin health and use
...
Haus-wide settings flow, not a Plugin-specific shortcut.
...
Core Haus owns the Plugin host boundary: lifecycle, enablement, Service
...
instead of spreading operation names through generic Haus docs.
...
Haus does not expose individual Plugin tool toggles.
...
controls inside historical renders are disabled; Haus does not use fallback
...
Google's OAuth client id is Haus-owned infrastructure config, not a Plugin
setting. Runtime reads the desktop client id and installed-app client secret from
environment supplied by Haus development or release packaging and stores only
