import type { HostedWorkspaceFileContent, HostedWorkspaceFileList } from '@tavern/api';
...
export async function listHostedAgentWorkspace(
    db: GrottoDatabase,
...
export async function readHostedAgentWorkspaceFile(
    db: GrottoDatabase,
    connections: ComputerConnections,
    member: GrottoUser | null,
    input: { agentId: string; path: string; serverId: string }
