...
import { type AvatarMediaType, avatarMaxBytes } from '@haus/api/avatar';
...
export function readAvatarBytes(bytesBase64: string, mediaType: AvatarMediaType): AvatarBytes {
    ...
    return { bytes, sha256: hashAvatarBytes(bytes) };
}

/** Provider-generated bytes skip upload validation but still need a digest. */
export function hashAvatarBytes(bytes: Uint8Array): string {
