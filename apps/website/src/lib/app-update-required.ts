 ...
 * data is served. It is Haus App's single typed "update required" signal.
 */
const updateRequiredTrpcCode = 'PRECONDITION_FAILED';

/** How Haus App resolves an update-required signal for the current shell. */
export type UpdateRequiredMode = 'desktop-update' | 'reload';

/**
 * A browser or current thin desktop shell reloads to fetch the current
 * Haus App. Older releases still carrying a bundled renderer must install an
 * App update instead.
 */
