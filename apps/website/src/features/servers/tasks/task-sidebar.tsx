...
export function TaskSidebar({
    ...
    const tasks = React.useMemo(
        () => tasksQuery.data?.map((item) => toTaskItem(item, humans)) ?? [],
        [humans, tasksQuery.data]
    );

    const activeView = resolveTaskView(searchParams.get('view'));
    const activeLabel = searchParams.get('label');
    const route = tasksRoute(slug);
    const viewHref = (view: TaskView) => taskFilterHref(route, view, activeLabel ?? undefined);
...
function taskFilterHref(route: string, view: TaskView, labelId?: string) {
