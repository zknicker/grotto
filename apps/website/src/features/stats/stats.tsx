import type { ComputerUsage } from '@tavern/api';
...
export function StatsSettings({ serverId }: { serverId: string }) {
...
export function Stats({
    ...
    computers: ComputerUsage[] | undefined;
    refreshError?: string;
                ...
                {status === undefined ? (
                    <div className="grid gap-6">
                        {computers?.map((computer) => (
                            <ComputerUsageCard computer={computer} key={computer.computerId} />
                        ))}
                    </div>
                ) : null}
            </SettingsSection>
...
function ComputerUsageCard({ computer }: { computer: ComputerUsage }) {
