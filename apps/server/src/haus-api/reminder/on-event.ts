import { reminderChangedEventSchema, reminderEventSubscriptionInputSchema } from '@haus/api';
...
export const onReminderEventProcedure = reminderProcedure
    ...
    .use(async ({ ctx, input, next }) => {
        await requireReminderOperator(ctx.hausDb, ctx.member, input.serverId);
        return next();
    })
