...
import { filterHostedReminders, toHostedReminderListItem } from './server-reminder-view-model.ts';
...
test('builds a script-redacted hosted reminder row', () => {
    const item = toHostedReminderListItem(reminder);
...
test('filters hosted reminders by Agent, status, and text', () => {
    ...
    expect(
        filterHostedReminders([reminder, canceled], {
            agentId: 'agt_cove',
            query: 'health',
            status: 'scheduled',
        })
