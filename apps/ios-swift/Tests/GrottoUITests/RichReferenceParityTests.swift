...
struct RichReferenceParityTests {
    ...
    private static let table: [Row] = [
        ...
        Row(
            markdown: "[the pull request](https://github.com/grotto/grotto/pull/56)",
            kind: .pullRequest,
            id: "https://github.com/grotto/grotto/pull/56",
            label: "#56",
            mark: .glyph(.pullRequest)
        ),
        Row(
            markdown: "[the release notes](https://grotto.dev/releases)",
            kind: .website,
            id: "https://grotto.dev/releases",
    ...
    /// phone shows its words. `grotto://` is what Agents are told to write.
    @Test func readsAnUnchippedLinkAsItsOwnWords() {
        #expect(
            RichMessageParser.parse("Open [the workspace](grotto://workspace/notes.md) later.") {
                _, _, _ in nil
            } == [
                .text("Open "),
                .link(text: "the workspace", target: "grotto://workspace/notes.md"),
                .text(" later."),
            ]
        )
        #expect(
            RichMessageParser.parse("[report.html](grotto://workspace/out/report.html)") {
                _, _, _ in nil
    ...
    @Test func leavesEveryUnchippableTargetAsALink() {
        let targets = [
            "mailto:ada@grotto.dev",
            "ftp://files.grotto.dev/notes.txt",
            "notascheme",
        ]
    ...
    /// kind names a Grotto record the phone has no route to.
    @Test func opensOnlyTheReferencesWhoseTargetIsAnAddress() {
        ...
        #expect(opens(.website, "https://grotto.dev"))
        #expect(opens(.pullRequest, "https://github.com/grotto/grotto/pull/56"))
        #expect(!opens(.agent, "agt_cove"))
        #expect(!opens(.channel, "cht_product"))
        // A workspace resource names an in-app target nothing opens yet.
        #expect(RichReferenceWireForm.activationURL(for: "grotto://workspace/notes.md") == nil)
        #expect(RichReferenceWireForm.activationURL(for: "notascheme") == nil)
        #expect(RichReferenceWireForm.activationURL(for: "mailto:ada@grotto.dev") != nil)
    }
    ...
    @Test func readsABarePullRequestAddressAsItsNumber() {
        let segments = RichMessageParser
            .parse("Merged https://github.com/grotto/grotto/pull/56/files") { _, _, _ in nil }

        #expect(segments.contains(.reference(RichReferencePresentation(
            id: "https://github.com/grotto/grotto/pull/56/files",
            kind: .pullRequest,
            label: "#56",
            avatarURL: nil
        ))))
